...
class DefaultToolGateway(IToolGateway):
    ...
    def register_provider(self, provider: ICapabilityProvider) -> None:
        ...
        # Mapping is rebuilt during list_capabilities(); keep invoke() fail-closed by default.
    ...
    async def invoke(self, capability_id: str, params: dict[str, Any], *, envelope: Envelope) -> Any:
        ...
        if provider is None:
            # Refresh mapping lazily; this keeps the MVP resilient when providers are dynamic.
