...
## Purpose
TBD - created by archiving change add-minimal-task-tools. Update Purpose after archive.
## Requirements
### Requirement: Minimal workspace file tools are available in framework2
The system SHALL provide tools named `read_file`, `search_code`, `write_file`, and `edit_line` for `dare_framework2` and expose them as tool capabilities via the tool gateway (e.g., capability IDs `tool:read_file`, `tool:search_code`, `tool:write_file`, `tool:edit_line` when using `NativeToolProvider`).

#### Scenario: Tool gateway lists the minimal toolset
- **WHEN** the tool gateway lists capabilities
- **THEN** it includes `tool:read_file`, `tool:search_code`, `tool:write_file`, and `tool:edit_line`

### Requirement: Workspace-root enforcement for file tools
All minimal file tools SHALL resolve input paths against `workspace_roots` from the effective config and MUST reject paths that resolve outside those roots.
...
Minimal file tools SHALL read guardrail settings from `Config.tools.<tool_name>` and apply safe defaults when unspecified or invalid.
...
Each minimal file tool SHALL include at least one evidence record describing the operation (tool name and target path) when it completes successfully.
