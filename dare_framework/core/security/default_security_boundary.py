...
class DefaultSecurityBoundary(ISecurityBoundary):
    ...
    - Anything above READ_ONLY requires explicit approval (APPROVE_REQUIRED).
    ...
    async def verify_trust(self, *, input: dict[str, Any], context: dict[str, Any]) -> TrustedInput:
        # The MVP keeps inputs unchanged but derives risk_level from trusted context where available.
        risk_level = context.get("risk_level")
        if isinstance(risk_level, RiskLevel):
            derived_risk = risk_level
        else:
            derived_risk = RiskLevel.READ_ONLY
        return TrustedInput(params=dict(input), risk_level=derived_risk, metadata={})
    ...
    async def execute_safe(self, *, action: str, fn: Callable[[], Any], sandbox: SandboxSpec) -> Any:
        # Sandbox is a stub in MVP; policy MUST be enforced before calling execute_safe().
        result = fn()
