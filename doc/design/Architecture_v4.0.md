# DARE Framework v4.0 架构设计（草案）

> 状态：草案（用于评审）
>
> 目标包结构：`dare_framework/`（最终收敛为单一架构；历史多版本目录后续清理）
>
> 本文描述 v4.0 的目标形态：它参考现有文档与代码，但不要求与当前实现 1:1 一致。
>
> 证据与可追溯：见 `doc/design/DARE_v4.0_evidence.yaml`（claims + sources + anchors）。
...
v4.0 的目标是：将项目收敛为 `dare_framework/` 下**单一一致**的架构与接口面，然后删除/归档历史迭代目录与冗余文档。
...
- **Layer 0（Kernel 控制面）**：长期稳定的“系统边界”。
  - 这些接口必须被所有编排实现复用。
  - v4.0 不要求提供 `IRunLoop/ILoopOrchestrator` 作为稳定接口。
...
> `doc/design/Interfaces_v4.0.md` 对应章节（见下表“详情章节”列）。
...
| security | trust/policy/sandbox 边界；审批策略与参数校验 | `ISecurityBoundary`（及其 policy/sandbox 子接口位） | `doc/design/Interfaces_v4.0.md` 的 `## 6. security（Trust + Policy + Sandbox）` |
| event | 可审计事件日志（WORM）与查询/重放 | `IEventLog` | `doc/design/Interfaces_v4.0.md` 的 `## 7. event（审计与重放）` |
| hook | 生命周期扩展点；best-effort hooks | `IExtensionPoint` | `doc/design/Interfaces_v4.0.md` 的 `## 8. hook（生命周期扩展点）` |
| config | 配置快照；Layer 3 managers（确定性装配） | `IConfigProvider`；（管理器接口位/约定） | `doc/design/Interfaces_v4.0.md` 的 `## 9. config（配置与 managers）` |
| memory / knowledge | 统一检索面；跨域组合（例如 knowledge 既是 retrieval 又可作为 tool） | `IRetrievalContext`；（组合位）“IKnowledge + ITool”等 | `doc/design/Interfaces_v4.0.md` 的 `## 10. memory / knowledge（统一检索面 + 组合接口位）` |
| Plan Tool | 控制类 capability：触发 re-plan；不作为普通副作用工具 | （约定/标记）`capability_kind=plan_tool` 等 | `doc/design/Interfaces_v4.0.md` 的 `## 11. Plan Tool（控制类工具）` |
...
> 详细类型/完整签名以 `doc/design/Interfaces_v4.0.md` 为准。
